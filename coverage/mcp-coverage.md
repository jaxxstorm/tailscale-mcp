...
| implemented | GET | `/tailnet/{tailnet}/logging/network` | `listNetworkFlowLogs` | tool: `tailscale_list_network_flow_logs` | `tool:tailscale_list_network_flow_logs` | Network flow logs are exposed through a bounded, cursor-based MCP tool to keep responses within client context limits. |
